...
local CreateFrame, GetSpellInfo, UnitGUID, UnitAura, UnitDebuff, GetTime = CreateFrame, GetSpellInfo, UnitGUID, UnitAura, UnitDebuff, GetTime
...
local function Check(unit, aura, filter, sourceGUID, destGUID)
    ...
    if not auraName and filter and not filter:match("HARMFUL") then
            ...
            if Tracker.LastTracked[spellID][sourceGUID][combatEventType].handled then
                ...
                if unit then
                    if Tracker.AuraFilters[spellID] and Tracker.AuraFilters[spellName] then
                        filter = filter .. " " .. MergeFilters({ Tracker.AuraFilters[spellName].AuraFilters[spellID] })
                    ...
                    if auraName then
                        ...
                        for otherDestGUID in pairs(Tracker.LastTracked[spellID][sourceGUID][combatEventType].unhandledGUIDs) do
                            Tracker.Auras[otherDestGUID][sourceGUID] = Tracker.Auras[destGUID][otherDestGUID] or {}
